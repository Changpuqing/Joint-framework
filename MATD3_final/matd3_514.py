...
class MATD3(object):
    ...
    def choose_action(self, obs, noise_std):
        ...
        a = (a + np.random.normal(0, noise_std, size=self.action_dim)).clip(0.1, self.max_action)
        return a
    ...
    def train(self, replay_buffer, agent_n):
        ...
        with torch.no_grad():  # target_Q has no gradient
            ...
            for i in range(self.N):
                ...
                batch_a_next = (batch_a_next + noise).clamp(0.05, self.max_action)
                batch_a_next = (batch_a_next).clamp(0.05, self.max_action)
