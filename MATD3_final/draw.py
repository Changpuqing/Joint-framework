...
class gogogo:
    ...
    def run_utility(self):
        ...
        self.file1 = self.smooth(np.load(r'E:\A2500\新建文件夹\MATD3_final\MATD3_830\MATD3_energy_evergy.npy', allow_pickle=True, encoding="latin1"), self.inter, self.weight)
        self.file2 = self.smooth(
            np.load(r'E:\A2500\新建文件夹\MATD3_final\MATD3_quanzhong_1003_1\MATD3_latency_average_1003_1.npy', allow_pickle=True, encoding="latin1"),
            self.inter, self.weight)
        self.file3 = self.smooth(
            np.load(r'E:\A2500\新建文件夹\MATD3_final\MATD3_quanzhong_120\MATD3_latency_average_120.npy', allow_pickle=True, encoding="latin1"),
            self.inter, self.weight)
        self.file4 = self.smooth(
            np.load(r'E:\A2500\新建文件夹\MATD3_final\MATD3_quanzhong_406_1\MATD3_latency_average_406_1.npy', allow_pickle=True, encoding="latin1"),
            self.inter, self.weight)
        plt.rcParams['font.sans-serif'] = ['SimHei']
        plt.rcParams['axes.unicode_minus'] = False
        # plt.style.use('bmh')
        # plt.rcParams['axes.fSacecolor'] = 'white'
        ...
        plt.ylabel('Average Latency', fontsize=15, fontproperties="Times New Roman")

        # 设置坐标轴刻度
        x_ticks = np.arange(0, 8001, 1000)
        # y_ticks = np.arange(-15, -1, 1)
        ...
        plt.plot(x4, self.file4, color='orange', linewidth=1.0, linestyle=':', label='Learning_rate=0.1')
        plt.plot(x3, self.file3, color='g', linewidth=1.0, linestyle=':', label='Alpha=120')
        plt.plot(x2, self.file2, color='blue', linewidth=1.0, linestyle=':', label='Alpha=80')
        plt.plot(x1, self.file1, color='red', linewidth=1.0, linestyle=':', label='Alpha=100')
        # 设置图例大小
        # plt.legend(fontsize=10, loc='best')
        plt.legend(loc='lower right', fontsize=10)  # 标签位置
        ax = plt.gca()
        handles, labels = ax.get_legend_handles_labels()
        plt.legend(handles[::-1], labels[::-1], loc='upper right', prop=font1)
        # plt.savefig('E:/A2500/新建文件夹/picture/Alpha_average_latency', dpi=500, bbox_inches='tight')
