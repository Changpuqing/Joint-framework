...
class EnvMove(object):
    ...
    def activity(self):
        ...
        for i in range(self.learning_windows): #生成总任务
            for j in range(self.UE_max_no):
                # tmp_buffer_size = np.random.randint(600000, 800000)
                # tmp_buffer_size = np.random.randint(400000, 600000)
    ...
    def get_state(self): #状态为任务大小，距离和队列积压
        ...
        state_t = [((np.sum(self.UE_buffer_total,axis=0)[0]+np.sum(self.UE_buffer_total,axis=0)[1]
                    +np.sum(self.UE_buffer_total,axis=0)[2]+np.sum(self.UE_buffer_total,axis=0)[3]
                    +np.sum(self.UE_buffer_total,axis=0)[4]))/5,self.ser_dis[0],(self.energy_q_con[0]+self.energy_q_con[1]
                                                                 +self.energy_q_con[2]+self.energy_q_con[3]
                                                                 +self.energy_q_con[4])/5]
